...
const EditableCell = ({
  ...
  row: { index },
  column: { id },
  handleDataUpdate, // This is a custom function that we supplied to our table instance
  ...props
}) => {
  ...
  const onChange = (e) => {
    // console.log(props);
    console.log("view", props.rows[index]);
    let val = e.target.value;
    let formId = props.rows[index].original._id;
    setValue(val);
    handleDataUpdate(index, id, val, formId);
  };

  // We'll only update the external data when the input is blurred
  const onBlur = () => {
    // handleDataUpdate(index, id, value,);
  };
...
const ReactEditableTable = ({ columns, data, handleDataUpdate, ...props }) => {
  ...
  return (
    <>
      <div
        id="selectable"
        className={`react-table-wrapper ${handleDataUpdate && "editable"}`}
      >
